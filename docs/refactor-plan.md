...
- **Flavor fidelity problems**: Generated LESS emits fixed hex constants (e.g., in `out/stylus_example.generated.less`), plus `generateCssTheme()` hard-codes fallbacks to `#ffffff`/`filter: brightness` rather than flavor tokens, so text colors stop following the selected flavor (issue #3). Handcrafted themes exclusively use `@base/@text/@surface*` tokens.
