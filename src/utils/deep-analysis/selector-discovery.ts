...
import type { SelectorInfo, SelectorGroup, SelectorCategory } from '../../types/deep-analysis';
...
function extractStyles(properties: string): SelectorInfo['currentStyles'] {
  ...
  const bgMatch = properties.match(/(?:^|;)\s*background(?:-color)?\s*:\s*([^;]+)/i);
  if (bgMatch) {
  ...
  const borderMatch = properties.match(/(?:^|;)\s*border(?:-color)?\s*:\s*([^;]+)/i);
  if (borderMatch) {
 ...
 */
function hasVisibleBackgroundStyle(styles: SelectorInfo['currentStyles']): boolean {
...
function detectCategory(selector: string): SelectorCategory {
  ...
  // Card detection
  if (lower.includes('card') || lower.includes('panel') || lower.includes('box')) {
    return 'card';
  }
  ...
  // Header detection
  if (lower.includes('header') || lower.includes('masthead')) {
    return 'header';
  }
  ...
  // Icon detection
  if (lower.includes('icon') || lower.includes('ico') || lower.includes('svg')) {
    return 'icon';
  }

  // Text detection (headings, paragraphs)
  if (lower.match(/^(h[1-6]|p|span|div|text)/)) {
    return 'text';
  }
...
export function filterByFrequency(
  ...
  return groups.map(group => ({
    ...
    totalCount: group.selectors.filter(s => s.frequency >= minFrequency).length,
...
export function getSelectorStats(groups: SelectorGroup[]) {
  ...
  return {
    ...
    avgSpecificity:
      totalUnique > 0
        ? groups.reduce(
            (sum, g) => sum + g.selectors.reduce((s, sel) => s + sel.specificity, 0),
            0
          ) / totalUnique
        : 0,
  };
  ...
  return groups.map(group => {
    ...
    group.selectors.forEach(selector => {
      ...
      const similar = group.selectors.filter(
        s => !seen.has(s.selector) && isSimilarSelector(selector.selector, s.selector, similarityThreshold)
      );
 ...
 */
function isSimilarSelector(sel1: string, sel2: string, threshold: number): boolean {
